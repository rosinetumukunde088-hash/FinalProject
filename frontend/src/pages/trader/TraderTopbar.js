    ...
    <div className="admin-topbar">
      <div className="admin-topbar-actions">
        <div className="admin-topbar-menu" ref={profileRef}>
          ...
          >
            <span className="admin-topbar-avatar" style={{ background: 'linear-gradient(135deg, #2563eb, #1d4ed8)' }}>{initial}</span>
            <span className="admin-topbar-username">{user?.name}</span>
            <FiChevronDown size={14} />
          </button>
          {profileOpen && (
            <div className="admin-dropdown admin-dropdown-right">
              <Link to="/trader/profile" className="admin-dropdown-item" onClick={() => setProfileOpen(false)}>
                <FiUser size={15} />
